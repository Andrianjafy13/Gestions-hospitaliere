import {
  LayoutDashboard,
  Settings,
  BarChart3,
  Share,
  CalendarDays,
  UserRoundPen
} from "lucide-react";
import { useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import hospitalIcon from "../../assets/icons.png";
import { useNotifPharmacie } from "./hooks/useNotifPharmacie";
import { ChatIcon } from "../ComponentsMessage/ChatIcon";

export default function NavBarPharm() {
  return (
    <>
      <Sidebar />
      <Topbar />
    </>
  );
}

/* =========================
   SIDEBAR
========================= */
function Sidebar() {
  const { nonVus, marquerVus } = useNotifPharmacie();
  const navigate = useNavigate();

  const handleClickOrdonnance = async () => {
    await marquerVus();
    navigate("/Pharmatie/delivranceOrdonance");
  };

  return (
    <aside className="hidden lg:flex w-64 flex-col bg-blue-900 text-gray-200 fixed inset-y-0 left-0 shadow-lg">

      {/* Logo */}
      <div className="flex items-center gap-3 px-5 h-16 border-b border-slate-700">
        <div className="w-8 h-8 bg-teal-500 rounded-xl flex items-center justify-center flex-shrink-0">
          <img src={hospitalIcon} alt="Hospital Icon" className="mt-3 w-8 h-8 mb-4" />
        </div>
        <div>
          <span className="font-bold text-white text-sm">HospitalMada</span>
          <p className="text-xs text-gray-300 -mt-0.5">Espace Pharmatie</p>
        </div>
      </div>

      {/* Navigation */}
      <nav className="flex-1 px-3 py-2 overflow-y-auto space-y-1 mt-4">

        <Link to="/Pharmatie/Dashboard">
          <MenuItem icon={<LayoutDashboard size={18} />} label="Dashboard" />
        </Link>

        <PharmatieMenu />

        <button onClick={handleClickOrdonnance} className="w-full text-left">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl
            cursor-pointer text-gray-300 hover:bg-slate-800 transition-all duration-200">
            <CalendarDays size={18} />
            <span className="text-sm">Délivrer ordonnance</span>
            {nonVus > 0 && (
              <span className="ml-auto bg-red-500 text-white text-xs font-medium
                rounded-full min-w-[20px] h-5 flex items-center justify-center px-1">
                {nonVus > 99 ? "99+" : nonVus}
              </span>
            )}
          </div>
        </button>

        <Link to="/pharmatie/statistiques-pharmaceutique">
          <MenuItem icon={<BarChart3 size={18} />} label="Statistiques" />
        </Link>

        <Link to="/pharmatie/settings">
          <MenuItem icon={<Settings size={18} />} label="Paramètres" />
        </Link>

        <Link to="/">
          <MenuItem icon={<Share size={18} />} label="Déconnexion" />
        </Link>

      </nav>
    </aside>
  );
}

/* =========================
   TOPBAR
========================= */
function Topbar() {
  return (
    // ✅ "relative" + "overflow-visible" — permet à FenetreChat (fixed) de s'afficher
    // ✅ z-30 — topbar au-dessus du contenu de la page
    <header className="relative overflow-visible sticky top-0 z-30 bg-white/90
      backdrop-blur-md border-b border-slate-100 h-16 flex items-center
      justify-between px-6 ml-64 shadow-sm">

      <h1 className="text-lg font-semibold text-gray-700">
        Tableau de bord Pharmatie
      </h1>

      <div className="flex items-center gap-4">

        <span className="text-sm text-gray-600">Pharmatie</span>

        <div className="w-9 h-9 rounded-full bg-teal-500 text-white flex items-center justify-center">
          <UserRoundPen size={18} />
        </div>

        {/* ✅ ChatIcon sans prop size — le composant gère sa propre taille */}
        <ChatIcon route="/pharmatie/Message"/>

      </div>
    </header>
  );
}

/* =========================
   MENU ITEM
========================= */
function MenuItem({ icon, label, active }) {
  return (
    <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer
      transition-all duration-200
      ${active ? "bg-teal-600 text-white font-semibold" : "text-gray-300 hover:bg-slate-800"}`}>
      {icon}
      <span className="text-sm">{label}</span>
    </div>
  );
}

/* =========================
   PHARMACIE MENU (dropdown)
========================= */
function PharmatieMenu() {
  const [open, setOpen] = useState(false);

  return (
    <div>
      <div
        onClick={() => setOpen(!open)}
        className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl
          cursor-pointer text-gray-300 hover:bg-slate-800 transition-all duration-200"
      >
        <div className="flex items-center gap-3">
          <CalendarDays size={18} />
          <span className="text-sm">Medicaments</span>
        </div>
        <span className="text-gray-400">{open ? "˄" : "›"}</span>
      </div>

      {open && (
        <div className="flex flex-col ml-6 mt-1 space-y-1">
          <Link to="/Pharmatie/Ajout-medicament">
            <div className="flex items-center justify-between px-3 py-2 rounded-lg
              text-gray-300 hover:bg-slate-800 text-sm transition-all duration-200">
              Ajout medicaments
              <span className="text-gray-400">›</span>
            </div>
          </Link>
          <Link to="/Pharmatie/ListeMedicament">
            <div className="flex items-center justify-between px-3 py-2 rounded-lg
              text-gray-300 hover:bg-slate-800 text-sm transition-all duration-200">
              Liste medicaments
              <span className="text-gray-400">›</span>
            </div>
          </Link>
          <Link to="/pharmatie/alertes-medicaments">
            <div className="flex items-center justify-between px-3 py-2 rounded-lg
              text-gray-300 hover:bg-slate-800 text-sm transition-all duration-200">
              Alertes médicaments
              <span className="text-gray-400">›</span>
            </div>
          </Link>
        </div>
      )}
    </div>
  );
}

/* =========================
   QUICK ACTIONS
========================= */
export function QuickActions() {
  const actions = [
    "Nouveau patient",
    "Nouvelle consultation",
    "Planifier rendez-vous",
    "Créer dossier médical",
    "Prescrire examen"
  ];

  return (
    <div className="bg-teal-500 p-4 rounded-xl flex gap-3 flex-wrap text-white">
      {actions.map((action) => (
        <button key={action}
          className="bg-white text-teal-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100">
          {action}
        </button>
      ))}
    </div>
  );
}

/* =========================
   STAT CARD
========================= */
export function StatCard({ title, value, color }) {
  return (
    <div className="bg-white p-5 rounded-xl shadow border-l-4" style={{ borderColor: color }}>
      <p className="text-sm text-gray-500">{title}</p>
      <h2 className="text-2xl font-bold text-gray-800 mt-1">{value}</h2>
    </div>
  );
}