...
export default function DelivranceOrdonnance() {
  ...
  const handleSupprimer = async () => {
    ...
    try {
      ...
      setOrdonnances((prev) =>
        prev.filter((o) => o.id !== itemASupprimer.id)
      );
  ...
  // ─────────────────────────────────────────────
  // Générer facture + redirection
  // ─────────────────────────────────────────────
  const handleFacture = async (o) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/insertion/pharmacie/facture",
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            patientNom: o.patientNom,
            medecinNom: o.medecinNom,
            consultationId: o.consultationId ?? o.id,
          }),
        }
      );
      ...
      if (!response.ok) {
        ...
        throw new Error(data.message || "Erreur facture");
      }

      // ✅ Redirection vers la page facture
      navigate("/Pharmatie/Facture", {
        state: {
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="bg-blue-900 px-5 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <span
              className="text-xs bg-blue-700 text-blue-100
              px-3 py-1 rounded-full"
            >
              {ordonnances.filter((o) => !o.vu).length}
              {" "}non délivrée(s)
            </span>

            <span
              className="text-xs bg-blue-800 text-blue-200
              px-3 py-1 rounded-full"
            >
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    ...
                    {o.vu && (
                      <button
                        onClick={() => handleFacture(o)}
                        disabled={loading}
                        className="text-xs bg-teal-500 text-white
                          px-3 py-1 rounded-lg hover:bg-teal-600"
                      >
                        💰 Facturer
                      </button>
                    )}
