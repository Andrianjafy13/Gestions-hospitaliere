...
export default function DelivranceOrdonnance() {
  ...
  const [filtre, setFiltre]           = useState("toutes"); // toutes | nonDelivrees

  // ✅ Marquer comme vu + charger au montage
  useEffect(() => {
    marquerVus();
    chargerOrdonnances();
  }, []);
            ...
            {ordonnancesFiltrees.map((o) => (
              <div key={o.id}
                className={`px-5 py-4 hover:bg-gray-50 transition
                  ${!o.vu ? "border-l-4 border-l-blue-500" : "border-l-4 border-l-gray-100"}`}>
